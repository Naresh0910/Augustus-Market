...
class User(db.Model, UserMixin):
    ...
    def prettier_budget(self):
        if len(str(self.budget))>=4 and len(str(self.budget))<6:
            return f'{str(self.budget)[:-3]},{str(self.budget)[-3:]}₹'
        elif len(str(self.budget))>=6:
...
class Item(db.Model):
    ...
    owner = db.Column(db.Integer(), db.ForeignKey('user.id'),unique=False, nullable=True)
