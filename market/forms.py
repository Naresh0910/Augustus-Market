...
class RegisterForm(FlaskForm):
    ...
    def validate_user_name(self, user_name_to_check):
        ...
        if user:
            raise ValidationError(
                'username already exits!please try different username')
    ...
    def validate_email(self, email_address_to_check):
        ...
        if email_address:
            raise ValidationError(
                'Email Address already exits!please try different email')
    ...
    password2 = PasswordField(
        label='*Re-Enter Password', validators=[EqualTo('password1'), DataRequired()])
...
class SellItemForm(FlaskForm):
    submit = SubmitField(label='Redirect')
